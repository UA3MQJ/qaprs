...
void PortoptionDialog::saveButtonClick() {
    ...
    switch ( portTypeBox->currentIndex()-1 ) {
        case 0 : //APRS INTERNET SERVER
                        ...
                        if (proxyCheckBox->isChecked()) {
                            query.exec( "insert into port_param_values (port_num, par_code, par_value) "
                                        "values("+portNumEdit->text()+",7,'T')");
                        } else {
                            query.exec( "insert into port_param_values (port_num, par_code, par_value) "
                                        "values("+portNumEdit->text()+",7,'F')");
                        };
                        if (proxyAuthCheckBox->isChecked()) {
                            query.exec( "insert into port_param_values (port_num, par_code, par_value) "
                                        "values("+portNumEdit->text()+",8,'T')");
                        } else {
                            query.exec( "insert into port_param_values (port_num, par_code, par_value) "
                                        "values("+portNumEdit->text()+",8,'F')");
                        };
                        query.exec( "insert into port_param_values (port_num, par_code, par_value) "
                                    "values("+portNumEdit->text()+",9,'"+proxyNameEdit->text()+"')");
                        query.exec( "insert into port_param_values (port_num, par_code, par_value) "
                                    "values("+portNumEdit->text()+",10,'"+proxyPortBox->text()+"')");
                        query.exec( "insert into port_param_values (port_num, par_code, par_value) "
                                    "values("+portNumEdit->text()+",11,'"+proxyUserEdit->text()+"')");
                        query.exec( "insert into port_param_values (port_num, par_code, par_value) "
                                    "values("+portNumEdit->text()+",12,'"+proxyPassEdit->text()+"')");
    ...
    emit savePort( portNumEdit->text().toInt() );
