...
*/
    if ( this->Core != NULL ) this->Core->atomCORE->wait();
...
void MainWindow::allLoaded( ) {
    ...
    while ( tmp != NULL ) {
        ...
        if ( pModel != NULL ) PacketsWindow.packetsModel.setData( PacketsWindow.packetsModel.index( i, 3 ), "(" + QString::number( tmp->Port_Num ) + ") " + pModel->data( pModel->index( tmp->Port_Num, 0, QModelIndex()), Qt::DisplayRole ).toString() );

        if ( pModel != NULL ) PacketsWindow.packetsModel.setData( PacketsWindow.packetsModel.index( i, 4 ), pModel->data( pModel->index( tmp->Port_Num, 1, QModelIndex()), Qt::DisplayRole ).toString() );
