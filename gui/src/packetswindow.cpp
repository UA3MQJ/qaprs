...
//
PacketsWindowImpl::PacketsWindowImpl( QWidget * parent)
        : QMainWindow(parent)
{
...
void PacketsWindowImpl::addPacket( QString TRX, int pK, int pNum, QString DT, QString To, QString From, QString Via, QString MsgText ) {
    ...
    if ( pModel != nullptr ) packetsModel.setData( packetsModel.index( 0, 3 ), "(" + QString::number( pNum ) + ") " + pModel->data( pModel->index( pNum, 0, QModelIndex()), Qt::DisplayRole ).toString() );

    if ( pModel != nullptr ) packetsModel.setData( packetsModel.index( 0, 4 ), pModel->data( pModel->index( pNum, 1, QModelIndex()), Qt::DisplayRole ).toString() );
