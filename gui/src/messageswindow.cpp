...
//
MessagesWindowImpl::MessagesWindowImpl( QWidget * parent)
        : QMainWindow(parent)
{
...
void MessagesWindowImpl::addMessage( QString TRX, int pNum, int K, QString DateTime, QString To, QString From, QString MsgText, QString MID ) {
    ...
    if ( pModel != nullptr ) messagesModel.setData( messagesModel.index( 0, 3 ), "(" + QString::number( pNum ) + ") " + pModel->data( pModel->index( pNum, 0, QModelIndex()), Qt::DisplayRole ).toString() );
    if ( pModel != nullptr ) messagesModel.setData( messagesModel.index( 0, 4 ), pModel->data( pModel->index( pNum, 1, QModelIndex()), Qt::DisplayRole ).toString() );
