...
class QAPRSPort;
class QAPRSAXIPPORT;
class QAPRSINTERNETPORT;
class QAPRSAGWPORT;
class QAPRSKISSPORT;
class QAPRSFLDIGIPORT;
class QAPRSVASKAPORT;
...
public:
       ...
       //QString      APRSCall( ); // { return QString("");/*sAPRSCall;*/ }
       //QAtomix *atomCORE;
       //QObject *atomCORE;
       QSysVars  *SysVars;
       ...
       QString      portTypeID() {return QString::number( this->iPortID ); } // возвращает тип порта

       void         setPortType( QString PType ) { sPortType = PType; } //
       void         setPortTypeID( int PType ) { iPortID = PType; } //
       ...
       bool stored; //признак того, что порт сохранен в хранилище
...
class QAPRSAXIPPORT: public QAPRSPort  {
    ...
    public:
       ...
       int           getParamCount();
       QString       getParamName( int parNum );
       QString       getParamValue( int parNum );
       void          updatePort();
