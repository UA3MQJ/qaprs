...
int  QAPRSPort::getParamCount() { return -1; };
QString QAPRSPort::getParamName( int parNum ) { Q_UNUSED(parNum) return ""; };
QString QAPRSPort::getParamValue( int parNum ) { Q_UNUSED(parNum) return ""; };
void QAPRSPort::updatePort() { }
void QAPRSPort::sendAPRSPacket( QString To, QString From, QString MsgText ) { Q_UNUSED(To) Q_UNUSED(From) Q_UNUSED(MsgText) }
void QAPRSPort::sendAXPacket( QByteArray AXPacket ) { Q_UNUSED(AXPacket) }

//QString QAPRSPort::APRSCall( ) { return sAPRSCall; };
...
void QAPRSAXIPPORT::setParam( QString ParamName, QString ParamValue ){
    ...
    if ( ParamName=="PORT_NAME" ) setPortName( ParamValue );

    //qDebug() << "now port name = " << portName();

    if ( ParamName=="RXPort" ) {
        if ((ParamValue.toInt()>0)&&(ParamValue.toInt()<65536)) RXPort = ParamValue.toInt();
        //qDebug() << "RXPort=" << RXPort;
    }

    if ( ParamName=="TXPort" ) {
        if ((ParamValue.toInt()>0)&&(ParamValue.toInt()<65536)) TXPort = ParamValue.toInt();
...
int QAPRSAXIPPORT::getParamCount() {

    return 3;

}

QString QAPRSAXIPPORT::getParamName( int parNum ) {
...
QString QAPRSAXIPPORT::getParamValue( int parNum ) {

    switch ( parNum ) {
        case 0 :
        return QString::number( RXPort );
        break;
        case 1 :
        return QString::number( TXPort );
        break;
        case 2 :
        return TXAddress->toString();
        break;
    }
...
void QAPRSAXIPPORT::readPendingDatagrams() {
    ...
    MsgText = decoder1->toUnicode( datagram.mid( sh + 2, datagram.count() - sh - 5 ) ); // +2 - skip Control byte and PID byte

    //qDebug() << "MsgText=" << MsgText;

    //qDebug() << "datagram=" << decoder1->toUnicode( datagram.mid( sh + 2, datagram.count() - sh - 5 ) );


    //qDebug() << "datagram=" << decoder->toUnicode( datagram.mid( sh + 2, datagram.count() - sh - 5 ) );



    //qDebug() << "DestCall=" << DestCall << " SrcCall=" << SrcCall << " MsgText=" << MsgText;
...
int QAPRSINTERNETPORT::getParamCount() {

    return 12;

}

QString QAPRSINTERNETPORT::getParamName( int parNum ) {
...
QString QAPRSINTERNETPORT::getParamValue( int parNum ) {
    ...
    switch ( parNum ) {
        ...
        break;
        case 6 :
        return (( UseProxy == true)? "T":"F");
        break;
        case 7 :
        return (( UseProxyAuth == true)? "T":"F");
        break;
        case 8 :
        return ProxyName;
        break;
        case 9 :
        return QString::number( ProxyPort );
        break;
        case 10 :
        return ProxyUser;
        break;
...
void QAPRSINTERNETPORT::sendAPRSPacket( QString To, QString From, QString MsgText ){
    ...
    if ( portStatus() == 0 ) {
        APRSPacket.clear();
        APRSPacket.append( From.left( From.indexOf('>') ) + ">" + SysVars->getVal( "APRSCall" ) +",TCPIP*:" + MsgText);
    ...
    while ( msg.length() > 0 ) {
       ...
       mesg = msg.left( msg.indexOf( 10 ) + 1 );
        ...
        if ( mesg.data()[ 0 ] != '#' ) {
            ...
            qDebug() << "portNum()=" << portNum() << " DestCall=" << DestCall << " SrcCall=" << SrcCall << " MsgText=" << MsgText;
            ...
            emit RXPacket( portNum(), true );
            emit RXPacket( portNum(), false );
            qDebug() << ("(" + QString::number( this->portNum() ) + ")" + this->portType() + "::SRVMsg='" + mesg.left(mesg.length() - 2) );
...
int QAPRSAGWPORT::getParamCount() {

    return 2;

}

QString QAPRSAGWPORT::getParamName( int parNum ) {
...
QString QAPRSAGWPORT::getParamValue( int parNum ) {
...
int QAPRSKISSPORT::getParamCount() {

    return 12;

}

QString QAPRSKISSPORT::getParamName( int parNum ) {
...
QString QAPRSKISSPORT::getParamValue( int parNum ) {
    ...
    switch ( parNum ) {
        ...
        break;
        case 1 :
        return QString::number( ComSpeed );
        break;
        case 2 :
        return Command1;
        break;
        ...
        break;
        case 6 :
        return QString::number( TXDelay );
        break;
        case 7 :
        return QString::number( TXTail );
        break;
        case 8 :
        return QString::number( SlotTime );
        break;
        case 9 :
        return QString::number( Persist );
        break;
        case 10 :
        return (( FullDuplex == true)? "T":"F");
        break;
        case 11 :
        return ExitFromKISS;
        break;
...
void QAPRSKISSPORT::openPort() {
    ...
    m_Handle = CreateFile( reinterpret_cast<const wchar_t *>( COMPortName.utf16() ), GENERIC_READ|GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, 0, nullptr);
    //m_Handle = CreateFile( (wchar_t *)(COMPortName.utf16()) , GENERIC_READ|GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, 0, nullptr);
...
//проверка, есть ли данные в буфере чтения порта
    ...
    if (ClearCommError(m_Handle, &e, &ComStat)) {
        ...
        if (aInQueue>0) {
            ...
            DWORD len = 1024;
            DWORD feedback = 0;
            ReadFile(m_Handle, buffer, len, &feedback, nullptr);
        ...
        ComReadData.clear();
            ...
            if (ComData.length()>0)     {
                ...
                while ( ComData.length()>0 ) {
                    ...
                    if ( ((ComData.data()[0])==char(0xC0)) && ((ComData.data()[1])==char(0x00)) ) {
...
int QAPRSFLDIGIPORT::getParamCount() {

    return 2;

}

QString QAPRSFLDIGIPORT::getParamName( int parNum ) {
...
QString QAPRSFLDIGIPORT::getParamValue( int parNum ) {
    ...
    if ( uchar(datain.data()[ datain.length() - 1 ]) == uchar(0x0d)) {
        ...
        while (msg.length()>0) {
            ...
            if (mesg.length()>1) {
                ...
                if ((DestCall.length()>0)&&(DestCall.length()<9)&&(SrcCall.length()>0)&&(MsgText.length()>0)&&(!((DestCall==SrcCall)))) {
...
int QAPRSVASKAPORT::getParamCount() {

    return 3;

}

QString QAPRSVASKAPORT::getParamName( int parNum ) {
...
QString QAPRSVASKAPORT::getParamValue( int parNum ) {
